...
#Retorna o dia da semana em formato numeral, sendo de 0 até 53
select date_format(curdate(), '%U') as data_formatada;

#Formatação para converter padrão universal em brasileiro
select date_format(curdate(), '%Y-%m-%d')as data_formatada;
select date_format(curdate(), '%d-%m-%d')as data_formatada;
...
#Funções para retornar somente o dia ou mês ou ano
...
select datediff('2001-11-20', '2026-05-21') as diferenca;
select year('2001-11-20') - year('2026-05-21') as diferenca;

#Calcula a diferença de horas
select hour(timediff('10:30','13:40')) as diferenca,
	concat('R$ ', hour(timediff('10:30','13:40'))*5)as valor;
